...
const addNewProducts= async (req, res)=>{
    try {
        ...
        const product = await db.createProduct(name, description, price, quantity, JSON.stringify(imageUrls),image,Number(sellerId));
...
const loadSellerRequestsPage=async (req,res)=>{
    try {
        const data=await db.getAllRequestedSellers();
        // res.json(allSellersData);
        // res.render("sellerRequestPage.ejs", {
        //     allSellersData
        // });
        const allSellersData=data.map((user)=>{
         delete user.password;
         return user;
        })
        console.log(allSellersData);
        res.status(200).json(allSellersData);
    } catch (error) {
        console.log(error);
    }
}
