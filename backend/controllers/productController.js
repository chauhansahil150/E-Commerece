...
//             INSERT INTO products (sellerid,name, description, price,quantity,thumbnail,images,is_verified,status)
//             VALUES (?,?, ?, ?,?,?,?,?,?)
//           `;
//           dbConnection.query(insertProductSQL,[1,ob.name, ob.description,ob.price,ob.quantity,ob.thumbnail,ob.images,1,'verified'], (error,results)=>{
//             if(error){
...
//         .catch(err=>console.log(err))
    ...
    const getProducts= async (req, res)=>{
        try {
            ...
            const products = await db.getAllProducts(userId,role,req.query.start,req.query.no_of_products);
            // const productCounts=await db.getTotalproductsCount();
            res.json(products);
          } catch (error) {
            console.error(error);
            res.status(500).json({ error: 'Internal Server Error ' });
