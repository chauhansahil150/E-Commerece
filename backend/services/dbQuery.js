...
const createProduct =  (name, description, price, quantity, imageUrl,image,sellerId) => {
    try {
      const insertProductSQL = `
      INSERT INTO products (sellerId,name, description, price,quantity,images,image,thumbnail)
      VALUES (?,?, ?, ?,?,?,?,?)
    `;

    dbConnection.query(insertProductSQL, [sellerId,name, description,price,quantity,imageUrl,image,image], (error,results)=>{
      if(error){
...
// Get all products with pagination
 function getAllProducts(userId,role,start, limit) {
    const lim = limit || 5;
    const offset = (start-1)*limit || 0
    // const products = await Product.find().(skipstart).limit(limit);
    return new Promise((resolve,reject)=>{
      console.log(role);
      let selectProductSQL;
      if(role=='seller'){
        selectProductSQL=`SELECT * FROM products  where sellerId=${userId} LIMIT ${lim} offset ${offset}`;
      }else if(role=='admin'){
        selectProductSQL=`SELECT * FROM products where status='verified' LIMIT ${lim} offset ${offset}`;
      }else{
        selectProductSQL=`SELECT * FROM products where status='verified' LIMIT ${lim} offset ${offset}`;
      }
...
async function updateProductById(productId, updatedData) {
  try {
    ...
    dbConnection.query(query,[updatedData.description], (error, results) => {
      if (error) {
        reject(error);
      } else {
        resolve(results);
...
// Update a user's password by ID
 function updateUserPassword(userId, newPassword,role) {
  return new Promise((resolve,reject)=>{
    let query;
    if(role=='user'){
       query=`
    ...
    `;
       ...
       query=`
    UPDATE delivery_man SET password=? WHERE id=?
    `;
    }
...
products.id as product_id,products.name,products.description,orders.cancel_date,orders.delivery_date,
products.thumbnail
from orders
